...
class Title {
  constructor({ gl, plane, renderer, text, textColor, font }) {
    ...
    this.text = text; this.textColor = textColor || '#545050'; this.font = font || '30px sans-serif'
    this.createMesh()
  }
  createMesh() {
...
class Media {
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius, font }) {
    ...
    this.borderRadius = borderRadius || 0; this.font = font
  ...
  update(scroll, direction) {
    ...
    if (this.bend === 0) { this.plane.position.y = 0; this.plane.rotation.z = 0 }
      ...
      const ex = Math.min(Math.abs(x), H), arc = R - Math.sqrt(R*R - ex*ex)
...
class GalleryApp {
  constructor(container, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase } = {}) {
    textColor = textColor || '#ffffff'; borderRadius = borderRadius || 0
    font = font || 'bold 30px sans-serif'; scrollSpeed = scrollSpeed || 2; scrollEase = scrollEase || 0.05
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    bend = bend !== undefined ? bend : 1
    const defaults = Array.from({length:6},(_,i)=>({image:`https://picsum.photos/seed/${i+1}/800/600`,text:`Photo ${i+1}`}))
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px sans-serif', scrollSpeed = 2, scrollEase = 0.05 }) {
